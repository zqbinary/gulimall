...
//        BoolQueryBuilder
        ...
        attr_id_agg.subAggregation(
                AggregationBuilders.terms("attr_name_agg").field("attrs.attrName").size(1)
        ).subAggregation(
                AggregationBuilders.terms("attr_value_agg").field("attrs.attrValue").size(50)
        );
        attr_agg.subAggregation(attr_id_agg);
        searchSourceBuilder.aggregation(attr_agg);
        log.info("构dsl：{}", searchSourceBuilder.toString());
     ...
     * @return
     */
    private SearchRequest buildSearchRequestOrigin(SearchParam param) {
    ...
    public SearchResult searchOrigin(SearchParam param) {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //2、执行检索请求
            SearchResponse response = esRestClient.search(request, GulimallElasticSearchConfig.COMMON_OPTIONS);
            //3、分析响应数据，封装成我们需要的格式
